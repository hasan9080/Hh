...
appropriate endpoint. The kubelet running on that node set up environment variable 
for each active service based on Service's virtual IP address and port. Similarly,
DNS creates set of DNS records for each service by watching kubernetes API.
