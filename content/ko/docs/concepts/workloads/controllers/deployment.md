...
* `.metadata.name` 필드에 따라, `nginx-deployment` 이름을 가진 디플로이먼트가 생성된다.
  ...
  [Writing a Deployment Spec](#writing-a-deployment-spec)를 참고하자.
...
1. `nginx:1.14.2` 이미지 대신 `nginx:1.16.1` 이미지를 사용하도록 nginx 파드를 업데이트 한다.
  ...
  ```
  여기서 `deployment/nginx-deployment`는 디플로이먼스를 지칭하며,
  `nginx`는 업데이트가 일어날 컨테이너를 가리키며
  `nginx:1.16.1`는 새로운 이미지와 그 태그를 지칭한다.
...
디플로이먼트의 `.metadata.name`은 컨트롤 플레인이 생성하는 디플로이먼트의 파드의 이름의 기초가 된다.
디플로이먼트의 이름은 유효한
[DNS 서브도메인](/ko/docs/concepts/overview/working-with-objects/names/#dns-서브도메인-이름)이어야 하지만,
이는 파드 호스트명에 예기치 못한 결과를 만들 수 있다. 최고의 호환성을 위해서는 이름이 
[DNS 레이블](/ko/docs/concepts/overview/working-with-objects/names#dns-label-names)의 엄격한 규칙을 따라야 한다.
