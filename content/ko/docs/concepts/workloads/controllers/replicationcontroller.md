...
컨트롤 플레인이 레플리케이션 컨트롤러의 파드를 새로 생성할 때, 레플리케이션 컨트롤러의 `.metadata.name`이 파드의 이름을 결정짓는 기초가 된다.
레플리케이션 컨트롤러의 이름은 유효한 [DNS 서브도메인 이름](/ko/docs/concepts/overview/working-with-objects/names/#dns-서브도메인-이름) 값이어야 하나, 의도하지 않은 파드 호스트명을 유발할 수 있다.
